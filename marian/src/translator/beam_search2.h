...
public:
  ...
  Beams toHyps(const std::vector<uint> keys,
    ...
    //std::cerr << "toHyps,beams_size:" << beams.size() << std::endl;//1
    //std::cerr << "toHyps,beamSize:" << beamSize << std::endl;//6
    std::cerr << "toHyps,keys.size:" << keys.size() << std::endl;
    Beams newBeams(beams.size());
    for(int i = 0; i < keys.size(); ++i) {
      int embIdx  = keys[i] % vocabSize;
      int beamIdx  = i / beamSize;
      std::cerr << "toHyps,keys[i]" << keys[i] << std::endl;
